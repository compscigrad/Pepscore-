    ...
    <>
      ...
      <main className="bg-black min-h-screen">
        ...
        <div className="max-w-[1200px] mx-auto px-6 py-10">
          <h1 className="font-heading text-[clamp(26px,4vw,38px)] font-bold text-white mb-3">Research Peptide Categories</h1>
          <p className="text-[15px] text-white/55 font-light mb-10 max-w-[640px]">
            Browse Pepscore Lab&apos;s catalog by category. Every compound is supplied for laboratory research use only.
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {groups.map((g) => (
              <Link
                ...
                className="bg-[#0d0d0d] border border-[#D4AF37]/15 rounded-2xl p-6 transition-all hover:-translate-y-1 hover:border-[#D4AF37]/40"
              >
                <h2 className="font-heading text-[17px] font-bold text-white mb-1">{g.category}</h2>
                <p className="text-[12px] text-white/45">{g._count._all} product{g._count._all === 1 ? '' : 's'}</p>
              </Link>
            ))}
          </div>
        </div>
