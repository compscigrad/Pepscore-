...
// Raised 2026-08-12 (revision pass #6, owner feedback: "fades too much as
// I scroll, want a lot more amino-acid-chain visibility"). `strong` is
// unchanged so the hero -- explicitly approved as-is -- renders
// identically; `medium`/`subtle` were roughly doubled so every other
// section carries real, visible presence instead of reading as a faint
// afterthought once you scroll past the first section.
  ...
  return (
    <div className={`absolute inset-0 overflow-hidden pointer-events-none ${className}`} aria-hidden="true">
      ...
      {/* Fade the art into the section's own black background at its
          ...
          behind it. Narrowed and lightened from the original pass --
          the old wide/dark fade was washing out too much of the
          artwork as you scrolled past each section. */}
      {fadeLeft && <div className="absolute inset-y-0 left-0 w-1/2 bg-gradient-to-r from-black via-black/40 to-transparent" />}
      {fadeRight && <div className="absolute inset-y-0 right-0 w-1/2 bg-gradient-to-l from-black via-black/40 to-transparent" />}
      {fadeBottom && <div className="absolute inset-x-0 bottom-0 h-1/4 bg-gradient-to-t from-black to-transparent" />}
      {fadeTop && <div className="absolute inset-x-0 top-0 h-1/4 bg-gradient-to-b from-black to-transparent" />}
    </div>
  )
}
