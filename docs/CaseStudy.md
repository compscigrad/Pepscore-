...
This case study is written for a professional portfolio and is grounded entirely in this repository's own record: `docs/Decisions.md` (42 logged engineering decisions), `docs/ChangeLog.md`, `docs/ComponentMap.md`, `docs/ProductRoadmap.md`, `docs/PaymentReadiness.md`, `docs/Architecture.md`, `docs/InvoiceSystemSpec.md`, the Prisma schema, and `git log` (138 merged PRs, chronological). Where the evidence is incomplete, ambiguous, or a claim can't be sourced, that is stated explicitly rather than filled in. No fabricated metrics, customer data, or accomplishments appear below. This document is maintained continuously — see "Continuous Update Rule" near the end.
...
Pulled from `docs/Decisions.md`'s 42 logged entries — the subset with the most architectural weight.
...
- **138 merged pull requests** (`git log --oneline`, chronological, PR #1 through #138).
- **41 logged architecture decisions** in `docs/Decisions.md`, each with Decision/Reason/Alternatives/Benefits/Drawbacks.
- **613 passing unit tests** across 58 test files (Vitest), grown from 47 tests at the introduction of the test framework.
...
- **Phase 3D — Global PepScore Lab notification/communication design system** (Planned, pinned to the roadmap as of this revision): a permanent requirement that every customer-facing communication surface (email, in-app portal notifications, admin alerts, SMS within its own constraints) present as one cohesive dark/gold PepScore Lab product via a shared branded email shell and design-token set, replacing each template's current independent styling — content/business logic unchanged, presentation only.
