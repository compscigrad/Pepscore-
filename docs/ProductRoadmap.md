...
- No customer-merge path once two `Customer` rows represent the same person but don't exact-match (auto-merge deliberately never applies on a weak match — by design, per `lib/customers.ts`). A hard dead end (raw `UPDATE` required) whenever it occurs. **Routed to Phase 4E — RESOLVED (2026-08-11, PR #193, Decision #74)**: `lib/customers/merge.ts` now provides a safe-merge-or-explicitly-blocked admin action, rendered directly on the duplicate-record banner. No open engineering gap remains.
...
**Update (2026-08-11, PR #193, Decision #74)**: the admin-operational customer-merge dead end routed here from 4D is resolved — `lib/customers/merge.ts` provides a full safe-merge-or-explicitly-blocked workflow (13 FK-bearing models reassigned, three real identity-conflict conditions detected and refused rather than guessed through), verified end to end against real Postgres. This closes the second and final genuine, non-owner-dependent engineering gap named in `docs/LaunchReadiness.md`'s Phase 4X report.
